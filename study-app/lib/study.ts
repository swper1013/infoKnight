...
export function getWeakTopics(
  ...
  questions.forEach((question) => {
    const wrongCount = questionProgress[question.id]?.wrongCount ?? 0;

    if (wrongCount > 0) {
      topicScores.set(question.topic, (topicScores.get(question.topic) ?? 0) + wrongCount);
    }
  });
...
export function generateTodayPlan({
  ...
  const heavyWrongReview = wrongQuestionCount >= 4 || totalWrongCount >= 6;
  ...
  const plans: Record<StudyStage, StudyBlock[]> = {
    ...
    middle: [
      ...
      },
      {
        ...
        description: "틀렸던 문제를 다시 풀고 해설에서 키워드만 다시 적어봅니다.",
      },
      {
        id: "memory-note",
        title: "암기 정리",
    ...
    final: [
      ...
      },
      {
        ...
        description: "오답노트 최상단 문제부터 다시 풀며 약점을 줄입니다.",
      },
      {
        id: "weak-topic",
        title: "취약 파트 보강",
